...
const faqs = [
  ...
  },
  {
    ...
    answer:
      "OpenSky and ADS-B One (optional live sources) return ADS-B for a bounded area around your observer and the map (same corridor rules; proxied on the server). Symbols are smoothed between refreshes on slower devices; OpenSky and ADS-B One also use short retention between snapshots on mobile. Static uses simulated points from routes.json. Heavy OpenSky use can hit anonymous rate limits (429); a free OpenSky account in server .env.local (OPENSKY_API_*, see .env.local.example) raises limits. ADS-B One is community-run and rate-limited (~1 request/s per IP); the app caches and debounces map moves. Route-region average-speed diagnostics use aircraft in the air with valid speed in the visible corridor. FlightRadar24 and other aggregators are not wired in — any external map can still differ from the feed you selected (different receivers, MLAT, partners).",
  },
  {
  ...
  },
  {
    ...
    answer:
      "The bar is a quick **height scale** (MSL): map symbols run from **light green** (low) through **green** into **blue** and **dark blue** at high cruise, by reported **baro/geo altitude**. Tick labels under the bar default to **short kilometre marks** (`0m`, `2k`, `4.5k` … = 0 m, 2 km, 4.5 km, and so on). **km** / **ft** on the **same row** as the legend title switch ticks to compact **thousands of feet** (labels only; colors and geometry stay in metres). **Bright green** (`#22c55e`) still marks **shot-feasible** aircraft — same meaning as elsewhere on the map (camera tools), even if that hue appears in the low-altitude part of the scale. The **checkbox** beside **Aircraft color by altitude (MSL)** turns the height scale off so other traffic uses one neutral tone; shot-feasible flights stay green. The **3D aircraft marker** also scales a bit with altitude (same height values).",
  },
  {
