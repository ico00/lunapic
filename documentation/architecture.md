...
**Performance:** `loadFlightsInBounds` runs on map **`moveend`** (and when **`flightProvider`** or **observer `lat`/`lng`** change). For **OpenSky**, `useMoonTransitMap` **debounces** bounds refresh (~800 ms, slightly longer when a flight is selected) so panning does not hammer the proxy. Don’t add synchronous heavy work in the main map thread without debounce.
