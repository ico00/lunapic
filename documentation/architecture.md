...
- `**/api/adsbone/point`** — Fallback: server-side `fetch` to `https://api.adsb.one/v2/point/{lat}/{lon}/{radiusNm}` (often blocked by Cloudflare on Vercel); short TTL cache. Primary path is browser → `api.adsb.one` from `AdsbOneFlightProvider`.
...
- **Flights** — Mapbox `model` layer with `model-id` loaded via `map.addModel` from `FLIGHT_3D_MODEL_URL` (placeholder airplane `.glb`). Rotation follows feature `track` (with a yaw alignment offset to match model forward-axis), altitude comes from `altitudeMeters` via `model-translation` Z. **`model-color`** uses **`flightFeatureColorMapboxExpression`** (`flightAltitudeColor.ts`): **`interpolate`** on `altitudeMeters` (MSL, light green → green → blue → dark blue) with **green** override when `isShotFeasible`; `model-color-mix-intensity` tints the GLB. **`model-scale`** also interpolates on altitude (larger at cruise) with zoom compensation. **`FlightAltitudeLegend`** (map bottom) documents the scale in English; tick labels can switch **km** vs compact **thousands of ft** via **`flightAltitudeLegendUnit`** in **`moon-transit-store`**, with the **km** / **ft** control on the **same row** as the legend title (`FlightAltitudeLegend.tsx`). Fallback **circle** layer reuses the same color expression if model setup fails; layer is `moveLayer`d to the top so it stays **above** stand and other GeoJSON overlays.
