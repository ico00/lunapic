...
2. **E2E** — Set **`data-testid="<feature>-select"`** (or similar) and **`data-value={current}`** on the trigger button so Playwright can assert value; for **`camera-sensor-select`** and **`camera-preset-select`**, click `role=option` by name. **Flight source** lists only **OpenSky** / **ADS-B One** checkboxes (`live-feed-opensky` / `live-feed-adsbone`) — no static/mock rows.
