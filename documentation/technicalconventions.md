...
- **Moon phase rendering is local, never a remote image.** `moonPhaseGeometry.ts` derives illumination and the bright-limb position angle and the viewfinder masks a bundled texture with them. Do **not** reintroduce hotlinked phase stills: the previous source (`svs.gsfc.nasa.gov`) went dark on 2026-08-16 and took the disk with it, and an `onerror` fallback does not save you — a hung connection stays silent until the browser's own connect timeout expires. Anything drawn in the field must render offline.
- **Ephemeris calls in render paths must be time-quantized.** `ViewfinderPreview` keys its phase `useMemo` on `Math.floor(simulatedEpochMs / 60_000)`, so a 4 Hz tick does not trigger 4 ephemeris evaluations per second. Illumination and limb angle drift far too slowly to justify it — see the CPU/fan overload incident of 2026-07-22.
