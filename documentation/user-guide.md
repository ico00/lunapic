...
4. **Choose a flight data source** — on a fresh load the app uses **OpenSky + ADS-B One together** (merged live fetch). Open the **Flight source** panel to toggle individual sources via checkboxes. The number in parentheses next to each source shows how many aircraft it currently sees.
  - **OpenSky** — live traffic for a **bounded region** around your **observer** and the **map view** (via the app’s server route; no OpenSky key in the browser). The map may **briefly retain** symbols between refreshes so tracking feels steadier on phones.
  - **ADS-B One** — alternate live feed (also proxied); same geometry rules; different upstream coverage and rate limits than OpenSky.
  - **LunaPic ADS-B** — **our own ADS-B receiver** (Raspberry Pi with a dedicated antenna, located at the observer’s base). Receives ADS-B signals directly over the air — no internet dependency for the actual RF reception. Shows only aircraft within antenna range (~200 km), but with **higher priority**: when LunaPic ADS-B and a web source both track the same aircraft, LunaPic ADS-B wins the position data (more current, no server-side latency). Aircraft disappear **immediately** when the Pi stops receiving their signal — no artificial retention delay. Enable alongside OpenSky/ADS-B One for best coverage, or alone for purely local real-time data.
...
5c. **Aircraft display mode (“Layers” tile)**  
  ...
  - Open the **Layers** control to choose the aircraft display mode (**3D Model**, **2D Flat**, **ATC Style**, **VFR Map**, **Street …**). The **small picture on the closed tile** shows the *other* mode — i.e. what you get if you tap and switch — not a duplicate of what is already active.  
  - The same popover has a **Flight history** section (available when the local ADS-B log is running): **Density heatmap (7 days)** and **Route lines (7 days)**. Route lines draw one thin semi-transparent line per **recorded flight pass** over the last 7 days — busy corridors emerge where many passes overlap; line colour follows the pass's median altitude (same ramp as live aircraft).  
...
5d. **Transit candidates panel** (icon with two arrows)
  - Shows flights predicted to cross or come close to the moon disc, split into two sections:
    - **Disk transit** — flights where the dead-reckoned trajectory at azimuth-alignment time places the aircraft within the moon’s apparent disc (`|Δalt| ≤ moon radius`). These are the actionable alerts.
    - **In frame** — flights within the camera’s vertical FOV but not predicted to overlap the disc. Useful context but lower priority.
  - Both sections only show flights whose transit is predicted to happen **within 100 km** of the observer. Planes currently in range but flying away — where the transit would occur far from you — are excluded.
...
7. **Read the “photographer” side** (wide layout: right column)
  ...
  - **Viewfinder preview** (when a flight is selected and timing data is available): a **3:2 black frame** with toggles **Full frame** (moon and plane scaled like your **effective focal** and **output size**) vs **Zoom** (normalized **half-degree** sky scale for quick comparison). The moon disk is drawn **on your device**, with no internet needed: a NASA full-moon photo is masked to the **phase** for the simulated time, and tilted to match how the terminator actually sits in your camera frame at your location. Its unlit half stays faintly visible so you can judge where the whole disk sits, not just the bright sliver — that dim half is a readability aid, not real earthshine, and the disk does not show **libration**. Footnote text under the preview explains scale, distance, and current illumination.
    - While the plane is still **outside the frame**, an edge arrow + angular separation (e.g. `4.5°`) show which way to look, and a **yellow outlined** silhouette on the moon previews the plane's expected **size** and **crossing direction** once it lines up — it's a simulation, not the plane's real position (that's the arrow).
...
8. **Optional AR field aid** (Field card → **Open AR sky overlay**)
  - Opens a fullscreen **rear-camera** view with projected labels for your **selected flight** and top current transit candidates.
  - Default mode also includes nearby live flights so labels appear while you scan the sky without manual map selection.
  - Use the AR mode toggle to switch between **Only focused** (selected + candidates) and **All nearby** (wider live set).
  ...
  - Aircraft labels include their elevation angle (e.g. `THY3VD +12°`). If an aircraft shows a low angle and you cannot see it, it may be near the horizon or too distant.
  - Aircraft outside the camera frame get **edge arrows**. Only aircraft **above the horizon** (elevation ≥ 0°) are shown.
  - A small **diagnostic bar** at the bottom shows: camera heading `↑ 085°`, tilt angle `∠ +32°`, flight data age in seconds, and moon azimuth/altitude. Use this to verify sensors are reading correctly.
  - The **compass badge** (± degrees) shows magnetometer accuracy. It turns red above ±15° — at that point a red warning appears and AR positions will be significantly off. Metal structures (cranes, window frames, reinforced concrete) cause this. Move away from them for accurate AR.
  - **Compass calibration (Cal ☽):** if the compass is disturbed, tap **Cal ☽** in the bottom bar, then tap where you actually see the Moon in the camera frame. The app computes the offset and corrects all labels. The button turns green and shows the applied offset (e.g. `Cal +24°`). Tap again to reset.
...
| **Aircraft markers (3D Model)**                              | Extrapolated position for the **simulated** time (and OpenSky display skew if you use it in the field tools). **3D model** orientation follows heading. **Color** = **altitude** (see the altitude legend) except **mint green** = shot-feasible in the camera tools. **Model scale** is **inverted** — lower-altitude aircraft (closer to the camera) appear larger; high-cruise traffic is smaller. Choose **ATC Style** from the **Layers** tile for a lighter 2D view — same data, different drawing. |
| **Green ring badge around an aircraft**                      | **Confirmed disk transit predicted.** The app has projected that aircraft's trajectory forward to the moment its compass direction matches the Moon, computed the Moon's position at that future instant, and verified that the aircraft will actually pass through the Moon's disk in the sky — not just cross the same compass bearing. No badge = azimuth may align, but the aircraft will pass above or below the Moon. See *Photographer panel — elevation gap* for details. |
...
**Golden / flash:** when alignment is very tight, the app can flash a gold cue (tolerance in the code is on the order of a tenth of a degree in full sky separation between moon and aircraft).

**Confirmed transit vs. azimuth-only alignment:** Most aircraft that share the Moon's compass direction will **not** actually cross the Moon disk — they pass above or below it because their elevation angle from your position doesn't match the Moon's altitude. The app distinguishes two situations:

- **Green ring badge on the map** — the aircraft's projected path, at the moment of compass alignment, will be within the Moon's angular diameter in full 2D sky space. This is the aircraft worth running to the balcony for.
- **No badge** — azimuth will align but the elevation will miss. The Photographer panel shows by how much (*"Elevation gap at alignment: −X.XX°"*) and in which direction (negative = aircraft passes below the Moon, positive = above).

The Moon's disk is only ~0.5° across. An elevation gap of even 1–2° is a complete miss. A gap of 44° (a real example) means the aircraft has no geometric chance of transiting regardless of where you stand.
...
- **Transit prediction is 3D, not azimuth-only:** the app computes the aircraft’s future position (dead-reckoned along its current heading and speed), looks up the Moon’s position at that future instant, then measures the **full angular distance** between aircraft and Moon on the celestial sphere (both azimuth and elevation). A transit is only confirmed when that distance is less than the Moon’s apparent radius (~0.26°). The “azimuth line” on the map shows compass direction only — an aircraft crossing that line on the ground may pass well above or below the Moon in the sky depending on its altitude and how far away it is.
- **Elevation gap explained:** for a given aircraft altitude and Moon elevation, there is exactly one distance from you at which the aircraft would appear at the same elevation angle as the Moon. Too far away = appears below the Moon; too close = appears above. The Photographer panel shows *”Elevation gap at alignment: ±X.XX°”* — how many degrees off the aircraft will be at the moment its azimuth matches the Moon. Negative means it passes below; positive means above.  
...
- **LunaPic ADS-B:** the Pi decodes 1090 MHz ADS-B transmissions directly — no internet round-trip for the RF data. Positions are real-time (updated every ~1 s on the Pi). When combined with web sources, LunaPic ADS-B position data **overrides** OpenSky/ADS-B One for the same aircraft. No retention: a plane vanishes from the map the moment it leaves antenna range or turns off its transponder.  
